...
class App:
    _conf = {                                               # === MAIN CONFIGS ===
        ...
        "rl": {                                             # === REINFORCEMENT LEARNER ===
            ...
            "training_steps": 1000,                         # Number of training steps (episodes) to run
            "epsilon_decay": 0.99,                          # Epsilon decay per training step
            "new_games_per_training_step": 7,               # Regulates when training runs (!)( >game_batch )
            "persist_model_per_training_step": 5,           # Persist model on each X 'th training step
        },
        "cnn": {                                            # === ARTIFICIAL (CONVOLUTIONAL) NEURAL NETWORK ===
            "learning_rate": 0.003,                         # Alpha
            "hidden_layers": (64, 64, 64),                  # Hidden conv-layers
        ...
        "topp": {                                           # === TOURNAMENT OF PROGRESSIVE POLICIES ===
            "visualize": False,                             # Render the board [ True | False]
            "matches": 200,                                 # Matches between a pair of agents as each player
        },
        "oht": {                                            # === ONLINE HEX TOURNAMENT ===
            ...
            "agent": "7x7/(1) CNN_S7_B1638.h5",             # Agent to use in OHT
        },
    }
