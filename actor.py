...
class Actor:
    ...
    def clear(self):
        """ Function that clears eligibilities and states visited before each episode """
        self.__eligibilities.clear()
        self.__states_visited.clear()

        # Adjust epsilon for each episode (this decays for each call)
    ...
    def set_eligibility(self, state: State, action: Action):
        """ Function that sets last visited (state, action) eligibility to 1 """
    ...
    def __adjust_eligibility(self, state: str, action: Action):
        """ Function that discounts the eligibility """
    ...
    def next_action(self, state: State, actions: ActionList) -> Optional[Action]:
        """ Return a action based on the highest SAP-value or epsilon exploration """
        if not actions:
            return None
    ...
    def __adjust_sap(self, state: str, action: Action, td_error: float):
        """ Function that discounts the SAPS values for a state and action """
